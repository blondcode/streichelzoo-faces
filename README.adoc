= Streichelzoo-Faces: Maven, Foundation based JSF Modules and Starter Kit
...
Then there is a theme Maven subproject, which depends on the Sass source maven jars.
Maven install compiles the Sass sources using gulp and provides a Maven theme jar module,
which is ready to use for a the webapp. Furthermore, it creates an theme archetype,
...
This results in this logical project structure:
...
Sass modules are used by the theme module. The theme provides processed css and js for the
webapp module. The webapp module contains the xhtml facelets and the Java backends.
It also makes use of fonts provided by the font modules. To create the project, run:
...
These archetypes contain the starter JSF2.2 webapp with its style and JavaScript framework
packages in separate modules. There can be multiple web applications sharing the
same base. In case of updates, for example a new Foundation for Sites version,
...
Wildfly and ++pom.xml++ contextPath for Jetty.
...
To set a new version, specify a local property version per artifact and use this:
 ...
 mvn versions:set versions:update-properties -DnewVersion=1.23
////
